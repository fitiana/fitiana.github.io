...
if (class_exists('WooCommerce')) {
    ...
    add_action('widgets_init', 'digi_product_variations_widget');

    function digi_product_variations_widget() {
        register_widget('Digi_Product_Variations_Widget');
    }
     ...
     * @extends  WC_Widget
     */
    class Digi_Product_Variations_Widget extends WC_Widget {

        public static $nasa_widget_id = 'nasa_woocommerce_filter_variations';

        public function __construct() {
            $this->widget_cssclass = 'woocommerce widget_layered_nav';
            $this->widget_description = esc_html__('Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'digi-theme');
            $this->widget_id = self::$nasa_widget_id;
            $this->widget_name = esc_html__('Nasa Product Variations Filter', 'digi-theme');
        ...
        public function init_settings($instance) {
            ...
            $this->settings = array(
                'title' => array(
                    'type' => 'text',
                    'std' => esc_html__('Filter by', 'digi-theme'),
                ...
                'attribute' => array(
                    ...
                    'std' => '',
                    'label' => esc_html__('Attribute', 'digi-theme'),
                    'options' => $attribute_array
                ),
                'query_type' => array(
                    'type' => 'select',
                    'std' => 'or',
                    'label' => esc_html__('Query type', 'digi-theme'),
                    'options' => array(
                        'or' => esc_html__('OR', 'digi-theme'),
                        'and' => esc_html__('AND', 'digi-theme')
                    )
                ),
                'show_items' => array(
                    'type' => 'text',
                ...
                'effect' => array(
                    'type' => 'select',
                    'std' => 'slide',
                    'label' => esc_html__('Effect for show more and show less', 'digi-theme'),
                    'options' => array(
                        'slide' => esc_html__('Slide Up-Down', 'digi-theme'),
                        'fade' => esc_html__('Fade In-Out', 'digi-theme')
                    )
                ),
                'count' => array(
                    'type' => 'checkbox',
        ...
        public function widget($args, $instance) {
            ...
            echo digi_get_content_widget_variation($args, $instance);
